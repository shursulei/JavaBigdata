package com.shursulei.sparklearning.sql.byJava;
...
//        Dataset peopleDS = sqlContext.read().json("file:///home/hadoop/people.json");
        DataFrame peopleDS = sqlContext.read().json("file:///home/hadoop/people.json");
...
//        Dataset excellentScore = sqlContext.sql("select name ,score from peopleScore where score >= 90");
        DataFrame excellentScore=sqlContext.sql("select name ,score from peopleScore where score >= 90");
        ...
        DataFrame peopleInformationDS = sqlContext.read().json(peopleInformationRDD);
...
//        Dataset excellentNameAgeDS =  sqlContext.sql(sqlText);
        DataFrame excellentNameAgeDS =  sqlContext.sql(sqlText);
        //将成绩优秀的人的成绩和年龄进行jion操作
        JavaPairRDD<String,Tuple2<Integer,Integer>> resultRDD = (JavaPairRDD<String, Tuple2<Integer, Integer>>) excellentScore.javaRDD().mapToPair(new PairFunction<Row,String,Integer>() {
...
//        Dataset personDS = sqlContext.createDataFrame(reusltRowRDD,structType);
        DataFrame personDS = sqlContext.createDataFrame(reusltRowRDD,structType);
        personDS.show();
